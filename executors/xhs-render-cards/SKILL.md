...
description: "把小红书草稿变成可发布文案和图文卡片的完整执行 SOP：锁定证据与账号角色、强制执行 DAI 改写、审定 image plan、获取真实浏览器素材、以参考图驱动重新生图、贴入真实截图，并通过自动与视觉质量检查。准备、渲染、修改或核验小红书图文时使用。禁止用 HTML、SVG 或模板生成确定性卡片；DAI 是本 Skill 内部的必要步骤。"
---
