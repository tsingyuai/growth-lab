...
2. 只读检查当前机器、进程环境、根目录 `.env.local` / `.env`、已声明的第三方 Client 目录和认证 profile。不得输出 secret、cookie、token 或认证文件内容。
3. 向用户给出一张统一状态表：`已就绪`、`缺失`、`可选`、`本轮绕过`。
4. 用自然语言询问用户要启用哪些缺失能力、哪些本轮绕过。不要替用户安装所有可选项。
5. 对用户选择启用的项目逐项处理：
   - API key：解释用途、获取地址和变量名，再请用户提供或在本地配置；写入 `.env.local` 前取得明确同意；永不回显完整值。
...
3. 确认后，让用户选择需要启用的平台：小红书、抖音、快手、B 站、微博、贴吧、知乎。未启用的平台必须由用户明确说“本轮绕过”，不得默认忽略。
...
- 替代关系必须显式呈现。例如生图只需 Gemini 或 OpenAI 其中一条；七个支持平台的搜索、详情、评论、媒体和创作者采集共用 MediaCrawler。
