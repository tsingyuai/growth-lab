---
name: xhs-replicate
description: 小红书内容增长闭环 —— 从调研找 idea、锚定爆款骨架、填入真实产品内容、生图与合规闸，到回收 24h/48h/7d 发布结果并反哺下一轮。用户要求出稿、复刻、月度复盘、查看发布数据或修订内容规律时使用。
---

# xhs-replicate — 1:1 复刻爆款 + 填充
...
- **高效** = 从「调研找 idea」到「发布就绪」的最短链路:锚一条已验证的爆款,复刻可迁移骨架,填入当前产品的真实内容并出图。产品事实必须来自用户当前 workspace 或明确提供的资料。
- **优质** = 两个指标驱动,**重心全在内容**:
...
```
① 找 idea  →  ② 复刻骨架  →  ③ 填内容  →  ④ 生图 + 闸  →  ⑤ 结果回收与反哺
```

### ① 找 idea(调研驱动)
- 调研**竞品 / 爆款 / 热点话题 / 热点事件**,产出「有曝光潜力的 idea」。
- 来源:[media-crawler-xhs](../../collectors/media-crawler-xhs/SKILL.md)(抓新爆款,守风控)+ `memory/xhs-replicate/libraries/xhs/` 中本 loop 运行时派生的通用候选样本 + 当下热点。仓库不附带私有对标库。
...
- 🔑 **选品判断不可省**：选对标爆款 = 这条流程成败的前移点。真正的错误不是“赛道不同”，而是没有识别差异便生搬：过气、擦边、钩子依赖当前任务无法成立的身份/承诺，或不可迁移部分恰好是其爆点。选品是人的判断，机械闸帮不上。

### ② 复刻骨架
- 锚 1 条爆款,1:1 复刻它的 **版式 / 钩子结构 / 卡编排 / 首页钩子**,不自创。
- 产出:`draft.md`(title/caption/tags 对标爆款打法)+ `image-plan.md` 的「**卡 ↔ 原图映射表**」(每卡对标原图第几张 / 版式照搬什么 / 大字换成什么 / 放哪张截图)。
- 实操由 [xhs-render-cards](../../executors/xhs-render-cards/SKILL.md) 总 SOP 负责，具体路线见其 `references/rendering.md`。

### ③ 填内容(优质核心 = 重心所在)
- 把当前产品的**已验证真实功能**填进复刻骨架；事实来源不明确就停下询问，禁止补写。
- 按以下钩子与价值约束填写:
...
### ④ 生图 + 机械闸
- **官号**:按 [xhs-render-cards](../../executors/xhs-render-cards/SKILL.md) 完成 DAI、image plan、素材、渲染和质量闸。**个人号**:仍必须完成 DAI；无图时可跳过渲染阶段。
- 出图 / 发布前过两道机械闸(`make lint-post`,exit 1 阻断):
...
用户说“这批发完看数据”“月度复盘”“反哺方法论”时，继续执行本 loop 的 [结果回收与反哺流程](references/result-intake.md)，不要转交独立的 `result-intake` Skill。
