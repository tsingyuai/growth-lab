...
| [小红书爆款复刻与反哺闭环](models/xhs-replicate/SKILL.md) | 协调采集、创作、降 AI 味、截图、生图、卡片渲染、合规机械闸和发布结果反哺。 | 保留通用 Skill 与可执行工具；不附带私有产品知识、样本、产物和运营 Memory。真实发布仍由人完成。 |

这些数字记录了一次实际运行结果，并作为后续迭代的证据。具体效果会受到产品、域名、搜索需求、页面质量、站点权重和观察窗口影响。

## 当前状态

Growth Lab 已实现 SEO 页面增长闭环，并提供小红书复刻与结果反哺工作流。可复用 Skill、脚本、CLI 和机械闸保存在各自能力目录；私有产品数据、样本、截图、生成帖子与历史 Memory 均不随仓库分发。小红书发布继续保持人工边界。
